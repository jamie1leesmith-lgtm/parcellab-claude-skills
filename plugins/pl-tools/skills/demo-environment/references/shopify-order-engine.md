...
```bash
PARCELLAB_ACCOUNT_ID="<manifest account.id>" EVENTS_DIR="orders/<nn>-<label>" GAP_SECONDS="<200 standard | 60 fast, from the manifest's run.pace>" \
...
absent `run.pace`) → 200, `fast` → 60. At `fast`, Beat 2's report must note
