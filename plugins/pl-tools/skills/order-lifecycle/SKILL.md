...
```bash
EVENTS_DIR="<run dir>" GAP_SECONDS="<gap, default 300>" \
...
- **Account:** every payload's `account` field and the driver's environment
  ...
  account.id> EVENTS_DIR="orders/<nn>-<label>" GAP_SECONDS="<300 standard |
...
- **Pace:** `GAP_SECONDS` is never hard-coded on an orchestrated run — it
  comes from the manifest's `run.pace` (`standard` → 300, `fast` → 60; an
